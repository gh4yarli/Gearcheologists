...
public class RedGoal_AprilTag extends Auto_CommonFunctions {
    ...
    public void runOpMode() {
        ...
        AprilTagMetadata metadata = tagLibrary.lookupTag(24);

        float[] tagPos = metadata.fieldPosition.getData();
        float tagX = tagPos[0];
        float tagY = tagPos[1];
        float tagZ = tagPos[2];

        telemetry.addData("Red tag ID 24", "X: %.2f, Y: %.2f, Z: %.2f", tagX, tagY, tagZ);
        telemetry.update();
