...
@Autonomous(name = "Goal_Spline", preselectTeleOp = "MecanumTeleOp")
@SuppressWarnings({ "unused" })
public class Goal_Spline_Pose extends Auto_CommonFunctions {
    ...
    // Target AprilTag IDs for pose correction (from AprilTagDriftMonitor)
    ...
    private Pose2d shootingPose; // Fixed reference pose for returning after each shot
    ...
    private void detectAllianceAndLocalize() {
        ...
        if (detectedTag != null) {
            ...
            // Use getAprilTagPose for initial pose setup
    ...
    private void firstShot() {
        updatePoseFromAprilTag();
        // Store the FIXED shooting pose - all subsequent shots will return here
        shootingPose = mecanumDrive.localizer.getPose();
        ...
        // Update pose after shooting but keep shootingPose as the fixed reference
        updatePoseFromAprilTag();
    }

    private void secondShot() {
        // Use current localizer pose for path start, NOT AprilTag (avoid reference
        // drift)
        ...
        double yTarget2 = isBlueAlliance ? 45 : -45;

        Action path = mecanumDrive.actionBuilder(currentPose)
                .splineToLinearHeading(splineTarget, splineHeading)
                .lineToY(yTarget)
                .lineToY(yTarget2)
                .splineToLinearHeading(shootingPose, shootingPose.heading.toDouble()) // Return to FIXED pose
                .build();
    ...
    private void fourthShot() {
        ...
        Action path = mecanumDrive.actionBuilder(currentPose)
                ...
                .lineToY(y1)
                .lineToY(y2)
                .strafeTo(shootingPose.position) // Return to FIXED shooting position
                .turnTo(shootingPose.heading.toDouble()) // Restore heading
                .build();
    ...
    private void aprilTagShoot() {
        ...
        ElapsedTime timer = new ElapsedTime();
        ElapsedTime blindRotationTimer = new ElapsedTime(); // Track blind rotation time
        final double MAX_BLIND_ROTATION_TIME = 0.5; // Stop blind rotation after 0.5 seconds (~90 degrees)
        ...
        while (opModeIsActive() && timer.seconds() < 0.8) {
            ...
            if (tag != null && tag.id == TARGET_ID) {
                ...
                sendPoseTelemetry();
                blindRotationTimer.reset(); // Reset blind rotation timer when tag is found
                if (Math.abs(headingError) < 1.5) {
                    break;
                }
            } else {
                // Only rotate blindly for a limited time to avoid over-rotation
                if (blindRotationTimer.seconds() < MAX_BLIND_ROTATION_TIME) {
                    mecanumDrive.setDrivePowers(new PoseVelocity2d(new Vector2d(0, 0), 0.2));
                } else {
                    mecanumDrive.setDrivePowers(new PoseVelocity2d(new Vector2d(0, 0), 0)); // Stop if still no tag
                }
                sendPoseTelemetry();
            }
        }
    ...
    /**
     * Get the current robot pose from AprilTag detection.
     * Uses the same pose mapping logic as AprilTagDriftMonitor.
     * Returns null if no valid goal AprilTag is detected.
     */
    private Pose2d getAprilTagPose() {
        List<AprilTagDetection> currentDetections = aprilTag.getDetections();

        for (AprilTagDetection detection : currentDetections) {
            if (detection.metadata != null &&
                    (detection.id == RED_TAG_ID || detection.id == BLUE_TAG_ID)) {
    ...
    /**
     * Update the robot pose from AprilTag if a valid goal tag is visible.
     * Uses the same pose mapping logic as AprilTagDriftMonitor.
     */
    private void updatePoseFromAprilTag() {
        Pose2d aprilTagPose = getAprilTagPose();
        if (aprilTagPose != null) {
            mecanumDrive.localizer.setPose(aprilTagPose);
            telemetry.addData("Pose Updated from AprilTag",
                    String.format("X: %.2f, Y: %.2f, Heading: %.2f°",
                            aprilTagPose.position.x,
                            aprilTagPose.position.y,
    ...
    public double moveToDesiredLocation(AprilTagDetection tag) {
        ...
        // Calculate turn power (heading correction) - matches Goal_Spline.java
        ...
        // Use MecanumDrive's setDrivePowers method - turn only, no strafe (matches
        // Goal_Spline.java)
