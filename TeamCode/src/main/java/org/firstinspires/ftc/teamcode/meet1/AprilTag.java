...
public class AprilTag extends LinearOpMode {
    ...
    // Target values
    final double DESIRED_DISTANCE = 60.0; // inches
    ...
    // Speed limits
    final double MAX_AUTO_SPEED = 0.1;
    final double MAX_AUTO_STRAFE = 0.1;
        ...
        if (opModeIsActive()) {
            while (opModeIsActive()) {
                ...
                if ((tagDetected==1)){
                    ...
                    // Sending the data for the values of the distance, yaw, and x
                    telemetry.addData("The distance is: ", desiredTag.ftcPose.y);
                    telemetry.addData("The yaw is: ", desiredTag.ftcPose.yaw);
                    telemetry.addData("The x is: ", desiredTag.ftcPose.x);

                    // Sending the data for the deltas for range, yaw, and strafe
                    telemetry.addData("The delta for distance is: ", rangeError);
                    telemetry.addData("The delta for the yaw is: ", headingError);
                    telemetry.addData("The delta for strafe is: ", strafeError);

                    // Use PID gains to determine move and turn power
                    double drive = Range.clip(rangeError * SPEED_GAIN, -MAX_AUTO_SPEED, MAX_AUTO_SPEED);
                    double turn =  Range.clip(headingError * TURN_GAIN, -MAX_AUTO_TURN, MAX_AUTO_TURN);
                    double strafe = 0; //Range.clip(-strafeError * STRAFE_GAIN, -MAX_AUTO_STRAFE, MAX_AUTO_STRAFE);
