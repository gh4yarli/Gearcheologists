...
public class RedLoadingSmallTriangle_v2 extends Auto_CommonFunctions {
    ...
    private void firstShot(){
        ...
        if (opModeIsActive()) {
            ...
            sleep(300);
            sleep(2000);
            //shootArtifactsSmallTriangle_FirstShot(launcher,intake1,intake2,arm,1700,ConfigurationConstants.SMALL_TRI_SHOOTING_TIME);
    ...
    private void secondShot(@NonNull MecanumDrive mecanumDrive){
        ...
        Action path_SecondShot = mecanumDrive.actionBuilder(pose)
                .splineTo(new Vector2d(-32, -30), Math.toRadians(-100))
                .lineToY(-62)
                .lineToY(-35)
                .splineToLinearHeading(new Pose2d(-60, -30, Math.toRadians(-22)), Math.toRadians(-90))
                .build();
    ...
    private void thirdShot_bigTri(@NonNull MecanumDrive mecanumDrive){

        mecanumDrive.updatePoseEstimate();
        Pose2d pose = mecanumDrive.localizer.getPose();
        // Third shot to go back to big triangle

        Action path_thirdShot = mecanumDrive.actionBuilder(pose)

                .splineTo(new Vector2d(-16,-25),Math.toRadians(-100))
                .lineToY(-62)
                .lineToY(-50)
                .setReversed(true)
                .splineToLinearHeading(new Pose2d(20,-20, Math.toRadians(-50)), Math.toRadians(-50))
                .build();
